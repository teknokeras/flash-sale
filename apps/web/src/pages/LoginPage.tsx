...
export default function LoginPage() {
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: () => saleApi.login(form),
        onSuccess: ({ token }) => {
            login(token, 'buyer')
            nav('/')
        },
        onError: (e: any) => setErr(e.error ?? 'Login failed'),
    })
    ...
    return (
        <div style={s.page}>
            <div style={s.card}>
                ...
                <p style={s.hint}>No account? <Link to="/register">Register</Link></p>
                <p style={s.hint}><Link to="/admin">Admin login →</Link></p>
            </div>
        </div>
    )
}
...
const s: Record<string, React.CSSProperties> = {
    ...
    hint: { textAlign: 'center', fontSize: 13, marginTop: 12, color: '#6b7280' },
}
